...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                timer.cancel();
                TimeChanger();
                if (choice == list.get(count).getCorrect()&& !isTimeOver) {
                    score += getScore;
                    scoreshow.setText("" + score);
                    Toast.makeText(getApplicationContext(), "CORRECT!", Toast.LENGTH_SHORT).show();
                    progress.setProgress(0);
                } else if(choice != list.get(count).getCorrect()) {
        ...
        timer = new CountDownTimer(30000,1000) {
            ...
            public void onTick(long millisUntilFinished) {
                j++;
                progress.setProgress((int)j*100/30);
                getScore = determineScore(j*100/30);
            }
    ...
    private int determineScore(int i) {
        if(i>=0&&i<25)
            return 200;
        else if(i>=25&&i<50)
            return 150;
